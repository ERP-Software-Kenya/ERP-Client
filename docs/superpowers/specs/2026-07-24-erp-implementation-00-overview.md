...
| Phase | File | Status |
|---|---|---|
| 0 | `2026-07-24-self-signup-design.md` + `docs/superpowers/plans/2026-07-24-self-signup.md` | **Done (2026-07-25).** Self-signup shipped (`36fa0e4`), 14 dead View.tsx files deleted (`eac0014`), unused deps removed (`98f706e`), `better-sqlite3`/`IpcResult` fix (`9a18c07`). Verified against working tree same day. |
| 1 | `2026-07-23-erp-implementation-01-crud-foundation.md` | **UI built (2026-07-25, `116c799`)**, all 12 full-CRUD pages exist. **Blocked, not verified done**: 8 of 12 resources 500 on list/mutation against the deployed backend for new orgs — narrowed to `core-apis`'s shared `BaseReadOnlyRepo.pagedAsync()`, exact crash line unconfirmed (see `docs/core-apis-fixes.md`), not a client bug. Needs a `core-apis` root-cause + fix + re-test before this phase can be marked complete. |
| 2 | `2026-07-23-erp-implementation-02-purchase-module.md` | **UI built 2026-07-26** (PO detail w/ line items, Bill detail w/ payments, Purchase Return via ItemReturns) — all disabled pending backend fixes. **Major finding**: PurchaseOrder's DTOs only ever exposed a `name` field (retroactively breaks Phase 1's `PurchaseOrders.tsx`/`Bills.tsx`/`PaymentTransactions.tsx` too, `core-apis-fixes.md` #0/#0c/#0d), and even the one DTO-clean resource (ItemReturns) 500s live — pointing at a shared root cause across most of the app, not isolated per-resource bugs. See the callout at the top of `docs/core-apis-fixes.md`. |
| 3 | `2026-07-23-erp-implementation-03-sales-module.md` | **UI built 2026-07-26** (Customer/Order/Invoice create forms, Sales Return via existing `ItemReturns.tsx`) — all disabled pending backend fixes. Resolved the spec's "single most important unknown": Orders do have a line-items concept server-side (`OrderItemEntity`), but it's schema-only like Phase 4's `StockTransferItemEntity` (`core-apis-fixes.md` #9). Customer/Order create 500 on a missing `organizationId` (`core-apis-fixes.md` #8) — a narrower bug than Phase 2's wholesale DTO mismatches. Invoice looks like the cleanest DTO in the whole investigation but is unverified and shipped disabled anyway. |
| 4 | `2026-07-23-erp-implementation-04-inventory-transactions.md` | **UI built 2026-07-25–26**, both pages disabled/stubbed pending backend fixes. Low-stock flag was already done in Phase 1. **Live-tested 2026-07-26** via an Electron+Playwright smoke run: both Stock Adjustment (`core-apis-fixes.md` #2) and Stock Transfer (`core-apis-fixes.md` #3) 500 on create against the deployed API — the latter contradicted source-level analysis that said it should work, corrected after live testing. Neither is usable until `core-apis` is fixed. |
