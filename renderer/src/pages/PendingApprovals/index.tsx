...
function customerLabel(item: CreditApprovalRequest) {
  const bill = item.bill;
  if (bill?.walkInName) return bill.walkInName;
  if (bill?.customer?.name) return bill.customer.name;
  if (item.customerId) return `Customer ${item.customerId.slice(0, 8)}…`;
  return 'Walk-in';
}

function amountOf(item: CreditApprovalRequest) {
  return Number(item.requestedAmount ?? item.amount ?? 0);
}
      ...
      <div className="flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Review credit sales that need approval.
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted border-b text-left sticky top-0">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold text-muted-foreground">Bill</th>
              ...
              {!list || list.length === 0 ? (
                ...
                list.map((item: CreditApprovalRequest) => (
                  <tr key={item.id} className="hover:bg-muted/50 transition">
                    ...
                    <td className="px-4 py-3 font-medium">{customerLabel(item)}</td>
                    <td className="px-4 py-3 text-xs text-muted-foreground font-mono">
                      {item.requestedById.slice(0, 8)}…
                    </td>
