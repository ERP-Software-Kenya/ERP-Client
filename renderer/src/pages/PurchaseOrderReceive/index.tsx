      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-3">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            {supplier && (
              <p className="mt-1.5 text-sm text-muted-foreground">{supplier.name}</p>
            )}
          </div>
        </div>
        ...
        {po.status === 'partially_received' && (
          <div className="flex items-start gap-2 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 px-3 py-2.5 text-xs text-amber-700 dark:text-amber-400">
            ...
            <span>
              Partially received. Inputs show remaining quantities — adjust before confirming.
            </span>
          </div>
        )}
        ...
        {po.status === 'partially_allocated' && (
          <div className="flex items-start gap-2 rounded-lg bg-violet-50 dark:bg-violet-950/30 border border-violet-200 dark:border-violet-800/50 px-3 py-2.5 text-xs text-violet-700 dark:text-violet-400">
            <Info size={13} className="shrink-0 mt-0.5" />
            <span>
              Some stock is already allocated. Assign the remaining received quantity below.
            </span>
          </div>
        )}
      </div>
      ...
      {showReceiveSection && (
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-border bg-muted/20">
            <div>
              <h2 className="text-sm font-semibold text-foreground">Step 1 — Record Receipt</h2>
              <p className="text-xs text-muted-foreground mt-0.5">Enter quantities physically received</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-border bg-muted/20">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Assign received stock to warehouse locations. Stock is added to inventory here.
              </p>
            </div>
          </div>
                    ...
                    <div key={item.id} className="px-5 py-4 space-y-3">
                      ...
                      <div className="flex items-center justify-between gap-3 flex-wrap">
                        ...
                        <div className="text-xs text-muted-foreground text-right">
                          ...
                          <span className="font-semibold text-foreground">{unallocated}</span>
                          {overAllocating && (
          ...
          <div className="px-5 py-4 border-t border-border space-y-3">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Warehouse size={16} />
