...
import { Locations, Products, UnpublishedStockPurchaseOrders, Suppliers } from '../../api';
import { getErrorMessage } from '../../lib/api-error';
import { FormSelect } from '../../components/FormSelect';
import type { UnpublishedStockPurchaseItem, PurchaseOrderStatus } from '../../types';
...
function hasUnallocated(items: UnpublishedStockPurchaseItem[]): boolean {
  return items.some(
    (item) => Math.max(0, Number(item.quantityReceived ?? 0) - Number(item.quantityAllocated ?? 0)) > 0,
  );
}

export default function BlackStockPurchaseOrderReceive() {
  ...
  const receiveMutation = UnpublishedStockPurchaseOrders.useReceive();
  const allocateMutation = UnpublishedStockPurchaseOrders.useAllocate();
  ...
  const handleAllocate = (): void => {
    ...
    allocateMutation.mutate(
      ...
      { onSuccess: () => navigate(`/black-stock/purchase-orders/${id}`) },
    );
  };
  ...
  if (isTerminal(po.status)) {
    return (
      <div className="space-y-5">
        <button
          type="button"
          onClick={() => navigate(`/black-stock/purchase-orders/${id}`)}
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── SECTION 2: ALLOCATE TO LOCATIONS ── */}
      {showAllocateSection && (
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-3.5 border-b border-border bg-muted/20">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Assign received stock to warehouse locations — this adds it to unpublished inventory
              </p>
            </div>
          </div>
