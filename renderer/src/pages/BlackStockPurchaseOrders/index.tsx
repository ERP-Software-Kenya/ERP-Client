...
import { UnpublishedStockPurchaseOrders, Suppliers } from "../../api";
import { usePagination } from "../../hooks/usePagination";
import { truncateId } from "../../lib/entityLabel";
import { loadErrorMessage } from "../../lib/api-error";
import type { UnpublishedStockPurchaseOrder, PurchaseOrderStatus } from "../../types";
...
export default function BlackStockPurchaseOrdersPage() {
  const navigate = useNavigate();
  const [deleteTarget, setDeleteTarget] = useState<UnpublishedStockPurchaseOrder | null>(null);
  const [supplierFilter, setSupplierFilter] = useState("");

  const removeMutation = UnpublishedStockPurchaseOrders.useDelete();
  const updateMutation = UnpublishedStockPurchaseOrders.useUpdate();
  ...
  const { data, isLoading, isError, error, refetch } = UnpublishedStockPurchaseOrders.useSearch({ page, filters });

  const listError = isError ? loadErrorMessage(error, "black stock purchase orders") : null;

  const columns: Column<UnpublishedStockPurchaseOrder>[] = [
    {
      key: "poNumber",
      label: "PO #",
      render: (row) => (
  ...
  return (
    <div className="space-y-4" style={{ height: "100%" }}>
      <DataTable
        title="Black Stock Purchase Orders"
        description="Manage black stock purchase orders. Click a row to view details or receive stock."
        columns={columns}
        ...
        onView={(row) => navigate(`/black-stock/purchase-orders/${row.id}`)}
        ...
        extraRowActions={(row) => {
          ...
          if (row.status === 'draft') {
            actions.push({
              ...
              onSelect: () => updateMutation.mutate({ id: row.id, body: { status: 'ordered' } as Partial<UnpublishedStockPurchaseOrder> }),
            });
          }
          if (
            row.status === 'ordered' ||
            ...
            actions.push({
              ...
              onSelect: () => navigate(`/black-stock/purchase-orders/${row.id}/receive`),
            });
          }
          return actions;
        }}
      ...
      <ConfirmDialog
        ...
        description="Delete this black stock purchase order? This cannot be undone."
